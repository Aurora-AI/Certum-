...
import { PHYSICS_WGSL, RENDER_WGSL } from './webgpu/shaderData';
    ...
    useEffect(() => {
        ...
        const init = async () => {
            ...
            for (let i = 0; i < PARTICLE_COUNT; i++) {
                ...
                // color (rgba)
                initialParticleData[off + 8] = 0.0; // r
                initialParticleData[off + 9] = 0.8; // g (Cyan-ish)
                initialParticleData[off + 10] = 1.0; // b
                initialParticleData[off + 11] = 0.5 + Math.random() * 0.5; // a
            ...
            // --- State Cycling (Morph) ---
            const simState = {
                shape: 0, // Reset to Sphere
                time: 0
            };

            interval = setInterval(() => {
                simState.shape = (simState.shape + 1) % 4;
                console.log("Morphing to Shape:", ["Sphere", "Shield", "Stream", "LOGO"][simState.shape]);
                console.log("Current Sim Params Buffer Update...");
            }, 4000);
            ...
            const render = () => {
                ...
                // Index 10 is target_shape (u32)
                u32[10] = simState.shape; 
                ...
                // Rotate camera slowly to see 3D Volume
                const camX = Math.sin(time * 0.2) * 20;
                const camZ = Math.cos(time * 0.2) * 20;
                mat4.lookAt(viewMatrix, [camX, 5, camZ], [0, 0, 0], [0, 1, 0]);
                ...
                const renderPass = commandEncoder.beginRenderPass({
                    colorAttachments: [{
                        ...
                        clearValue: { r: 0.0, g: 0.02, b: 0.05, a: 1.0 }, // Deep Void
                        loadOp: 'clear',
                        storeOp: 'store',
                    }],
                });
